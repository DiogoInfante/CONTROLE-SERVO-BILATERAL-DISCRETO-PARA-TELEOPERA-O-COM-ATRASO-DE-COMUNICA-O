function [Xd_m, U_m, X_m, Y_m, Xest_m, Erro_m, U_s, X_s, Y_s, Xest_s, Erro_s] = simuladorMestreEscravo(modeloEspacoEstadoDiscreto, manipulador, trajetoria, f_ext, Lp_m, Lp_s, K_m, K_s, t)
...
for t0 = t
   ...
   % Jacobian
   J = [l1*cos(X_s(1)) - cos(X_s(3))*(l2 - l4);
        l1*sin(X_s(1)) - sin(X_s(3))*(l2 - l4)];
   
   % Torques Externos no Escravo
   U_ext = 0;
   if k >= 125 && k<=250  
        U_ext = transpose(J)*f_ext;
   end
